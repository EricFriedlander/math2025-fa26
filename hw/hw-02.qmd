...
-   Make a folder called `hw`to store all of your homework for the semester. Create a subfolder called `hw02` to store this homework.

-   Log into [Canvas](https://cofi.instructure.com/courses/18743/assignments/243285), navigate to HW-02 and upload the `hw-02.qmd` file into the folder you just made and `parks.csv` into the `data` folder.
...
In your `hw-02.qmd` document you'll see that each task is wrapped in an `exercise` box, with an empty `answer` box below it for you to fill in your response. Use this format for the remaining exercises.
...
:::{.exercise}
Load the `parks.csv` data into R. Remember that you may need to change the PATH which you can accomplish by deleting the existing path, pressing the TAB button while your cursor is in between the quotes, and then selecting the file you want to load.
:::
...
Answer the following questions about the `parks` dataset using skills you've learned so far.
...
::: {.exercise}
The primary explanatory/predictor variable for this analysis `spend_per_resident_data` is quantitative; however, from the glimpse of the data earlier, we see its data type is `chr` (character) in R. We would expect it to be `dbl` (double), the data type for numeric data.
...
Don't write any code for this question, we will fix the issue in the next exercise.
:::
...
::: {.exercise}
The code below to mutates `spend_per_resident_data` so that it is correctly treated as quantitative data in R. Each line of code is numbered. Write a brief explanation of what each line of code does. You may use the internet or AI here to figure out the answer. If you do, please mention what source you used. A good technique is to put the following into your favorite AI chatbot:

> I am trying to learn R for the first time. Can you explain what each line of code here is doing?
> \[Paste the code from below\]

Note that if you use AI this way, I ask that you mention which source you used and retinterpret the answer you receive to ensure that you understand the result. DO NOT DIRECTLY COPY AND PASTE THE ANSWER YOU RECEIVE FROM AI.
...
::: callout-tip
This is a good place to render your hw-02 document.
:::
...
Fill in the code below to visualize the distribution of `spend_per_resident_data` using a histogram and calculate summary statistics for the variable using the `favstats` function from the `mosaic` R package.
...
::: {.exercise}
The `parks` data set ranges from the year 2012 to 2020, so each row contains information about funding and public amenities for each city in a given year. Thus, there are multiple rows in `parks` for each city. We are going to learn about two new `dplyr` verbs: `group_by` and `summarize`.

- `summarize`: This verb computes summary statistics. For example, `summarize(mean_spend = mean(____))` computes the mean of the `____` variable for each group.
- `group_by`: This verb groups the data by a given variable and you typically want to use it before `summarize`. When you group by a one (or more) variable(s), summarize will compute the summary statistics for each group. Consider the following code (don't remove the `#|eval: FALSE` lines):
...
student_data |>
  group_by(major) |>
  summarize(avg_grade = mean(gpa),
            median_grade = median(gpa))
```

This code would take a dataset called `student_data` and return a dataset with one row for each major, containing the average GPA and median GPA for each major as columns.

We are going to summarize the data, so there is just one row per city. Fill in the code below to create a new data frame called `parks_summary` that contains the mean spending per resident and mean points in the ParkScore index for the percent of residents within a 10-minute walk to a park.
...
parks_summary <- ____ |> # insert data set
  ...
  summarise(mean_spend = ____, # insert name of variable you want to compute the mean of
            mean_pts_near = ____) # use the previous line to do the same with the second variable
```
:::

::: callout-important
Use `parks_summary` for exercises 9 - 10.
:::

::: {.exercise}
-   Create a visualization of the relationship between `mean_spend` and `mean_pts_near` between the two variables, including a line. Include an informative title and informative axis labels on the visualization.
...
::: {.exercise}
-   Fit a linear regression model of the relationship between `mean_spend` and `mean_pts_near`. Think carefully about which one should be the response variable. Use the `lm` function and display the model in a tidy format.
