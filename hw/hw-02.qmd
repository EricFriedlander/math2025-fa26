...
-   Go to <{{< var rstudio_url >}}> and login with your College of Idaho Email and Password.

-   Make a subfolder called `hw02` in your `hw` directory to store this homework.

-   Log into [Canvas](https://cofi.instructure.com/courses/CANVAS_ID_TBD), navigate to Homework 2 and upload the `hw-02.qmd` and `us-counties-sample.csv` files into the folder you just made.
...
You are asked to use a reproducible workflow for all of your work in the class, and the goal of this question to is better understand potential real-world implications of doing (or not) doing so. [Here](/slides/01-big-picture.html#when-things-go-wrong) are some real-life examples in which having a non-reproducible workflow resulted in errors that impacted research and public records.
