...
-   Some based data clearning techniques
...
::: {.cell}
::: {.cell-output-display}
![](03-slr-intro_files/figure-revealjs/unnamed-chunk-3-1.png){width=80%}
:::
:::
...
- Suppose we want to add a third variable to our plots, such as the `season` variable. We have a few options:
    + Color the points based on the third variables (add `color = ~season` to the `gf_point` call)
...
::: {.cell-output-display}
![](03-slr-intro_files/figure-revealjs/unnamed-chunk-4-1.png){width=80%}
:::
:::
...
::: {.cell-output-display}
![](03-slr-intro_files/figure-revealjs/unnamed-chunk-6-1.png){width=80%}
:::
:::
...
- To keep some observations use `filter(true/false statement)` and filter will keep all rows where the inside is true.
Examples:
    - `Thing1 > 0`: keeps all days where the temperature was above 0 degrees Celsius
    - `Thing2 <= 1000`: keeps all days where the number of bike rentals was less than or equal to 1000
    - `Thing1 == Thing2`: keeps everything where `Thing1` equals `Thing2`?
    - `Thing1 != Thing2`: keeps everything where `Thing1` isn't equal to `Thing2`?
- Each group: Choose a season.
- Do Exercises 7 & 8
...
# A tibble: 2 × 5
  ...
  <chr>          <dbl>     <dbl>     <dbl>        <dbl>
1 (Intercept)    2077.     476.       4.37 0.0000214   
2 temp_orig       153.      26.6      5.75 0.0000000387
```
...
|term        |  estimate| std.error| statistic|  p.value|
|:-----------|---------:|---------:|---------:|--------:|
|(Intercept) | 2077.4771| 475.67243|  4.367453| 2.14e-05|
|temp_orig   |  152.8729|  26.58745|  5.749815| 0.00e+00|
...
::: {.cell-output-display}
![](03-slr-intro_files/figure-revealjs/unnamed-chunk-14-1.png){width=80%}
:::
:::
...
- Slope: For each addition degree Celcius we expect the number of bike rentals to increase by 152.8729035, on average, in the Fall.
- Intercept: If the temperature were 0 degrees Celcius, we predict the number of bike rentals to be 2077.4770818 in the Fall.
